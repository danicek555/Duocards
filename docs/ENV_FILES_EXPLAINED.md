...
- `SKIP_EMAIL_VERIFICATION=true` (optional, for development)

**When used:** Always loaded first, acts as fallback defaults

**Note:** Set `SKIP_EMAIL_VERIFICATION=true` in your `.env` or `.env.development.local` to disable email verification during registration. This is useful for development.
