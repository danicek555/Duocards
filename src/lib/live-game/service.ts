...
// This is the internal Next.js fallback for the live game backend. It is a
// faithful port of the standalone Fastify route logic (backend/src/routes/
// live-sessions.ts): the game engine, tokens and contract are reused verbatim;
// only the HTTP glue (Fastify request/reply, requireAuth, ApiError) is adapted.

const AUTH_SECRET = process.env.AUTH_SECRET || process.env.NEXTAUTH_SECRET || "";

/** Mirrors the shared backend ApiError so the client error envelope matches. */
...
function verifySessionToken(
  ...
  const token = bearerToken(authHeader ?? undefined);
  const payload = verifyLiveGameToken(token ?? undefined, AUTH_SECRET);
  if (
    !payload ||
...
async function loadSnapshot(
  ...
  if (!session) {
    throw new LiveApiError(404, "LIVE_SESSION_NOT_FOUND", "Live session not found");
  }
  ...
  const status = selfPacedMode
    ? await lazyFinishSelfPaced(session.id, session.status, endsAt, now)
    : session.status;
  ...
  const viewerSelfPaced =
    selfPacedMode && viewerParticipantId
      ? await (async () => {
          const answered = await prisma.liveAnswer.count({
            where: {
              participantId: viewerParticipantId,
          ...
          const nextRound =
            status === "QUESTION" && answered < totalQuestions
  ...
  return {
    ...
    currentQuestion: currentRound
      ? {
          ...
          answeredCount,
          ...(revealAnswer ? { correctAnswer: currentRound.correctAnswer } : {}),
        }
      : null,
...
function resolveNickname(requested: string, existing: readonly string[]) {
  ...
  if (!base) {
    throw new LiveApiError(400, "LIVE_NICKNAME_INVALID", "Nickname is required");
  }
...
async function assertPlayerIsActive(sessionId: string, participantId: string) {
...
export async function createLiveSession(userId: number, body: CreateSessionInput) {
  if (!isLiveGameModeId(body.modeId)) {
    throw new LiveApiError(
      400,
  ...
  const ownedSets = await prisma.flashcardSet.findMany({
    where: { id: { in: body.flashcardSetIds }, userId },
    select: { id: true, words: { select: { word: true, translation: true } } },
  });
  if (ownedSets.length !== body.flashcardSetIds.length) {
    throw new LiveApiError(
      404,
  ...
  const modeId: LiveGameModeId = body.modeId;
  const questionCount =
    modeId === "sprint" ? SPRINT_QUESTION_COUNT : body.questionCount;
  ...
  const durationMinutes =
    modeId === "marathon"
      ? Math.min(
          body.durationMinutes ?? MARATHON_DEFAULT_DURATION_MINUTES,
          MARATHON_MAX_DURATION_MINUTES,
        )
      : undefined;
  const roomCode = await generateAvailableRoomCode();
  const settings = {
    flashcardSetIds: body.flashcardSetIds,
    questionCount: questions.length,
    questionTimeSeconds: body.questionTimeSeconds,
    ...(durationMinutes !== undefined ? { durationMinutes } : {}),
    answerMode: body.answerMode ?? "choice",
  ...
  return { session: await loadSnapshot(session.id), hostToken };
}

export async function joinLiveSession(body: JoinSessionInput) {
  const roomCode = normalizeLiveGameRoomCode(body.roomCode);
  const session = await prisma.liveSession.findUnique({
    where: { roomCode },
    include: {
      participants: {
  ...
  if (session.participants.length >= 50) {
    throw new LiveApiError(409, "LIVE_SESSION_FULL", "The live session is full");
  }
  ...
  const playerToken = createLiveGameToken(
    { sessionId: session.id, role: "PLAYER", participantId: participant.id },
    AUTH_SECRET,
  );
  ...
  return {
    participant,
    playerToken,
    session: await loadSnapshot(session.id, participant.id),
  };
}
...
export async function getLiveSession(
  ...
  const token = verifySessionToken(authHeader, sessionId);
  if (token.role === "PLAYER") {
    await assertPlayerIsActive(sessionId, token.participantId!);
  ...
  return {
    session: await loadSnapshot(sessionId, token.participantId ?? undefined),
  };
}
...
export async function startLiveSession(
  ...
  verifySessionToken(authHeader, sessionId, "HOST");
  const session = await prisma.liveSession.findUnique({
    where: { id: sessionId },
  ...
  if (isSelfPacedModeId(session.modeId)) {
    ...
    const claimed = await prisma.liveSession.updateMany({
      where: { id: sessionId, status: "LOBBY" },
      data: {
        status: "QUESTION",
        startedAt: now,
        settings: {
    ...
    return { session: await loadSnapshot(sessionId) };
  }

  const locksAt = new Date(now.getTime() + firstRound.timeLimitSeconds * 1_000);
...
export async function submitLiveAnswer(
  ...
  const token = verifySessionToken(authHeader, sessionId, "PLAYER");
  const participantId = token.participantId!;
  await assertPlayerIsActive(sessionId, participantId);
  ...
  if (existingByKey) {
    return {
      ...
      session: await loadSnapshot(sessionId, participantId),
    };
  }
  ...
  const [session, round, previousAnswer, participant] = await Promise.all([
    ...
    prisma.liveRound.findFirst({
      where: { id: body.roundId, sessionId },
    }),
    prisma.liveAnswer.findUnique({
      where: {
        roundId_participantId: { roundId: body.roundId, participantId },
      },
    }),
    prisma.liveParticipant.findUnique({ where: { id: participantId } }),
  ]);
  if (!session || !round || !participant) {
    throw new LiveApiError(404, "LIVE_ROUND_NOT_FOUND", "Live round not found");
  }
  if (previousAnswer) {
    throw new LiveApiError(
      409,
  ...
  if (isSelfPacedModeId(session.modeId)) {
    ...
    if (
      ...
      throw new LiveApiError(409, "LIVE_ROUND_CLOSED", "The round is closed");
    }
  ...
  return {
    created: true,
    answer: { accepted: true as const, correct: isCorrect, points },
    session: await loadSnapshot(sessionId, participantId),
  };
}
...
export async function advanceLiveSession(
  ...
  if (!currentRound) {
    throw new LiveApiError(500, "LIVE_ROUND_NOT_FOUND", "Live round not found");
  }
...
export async function selectLiveTeam(
  sessionId: string,
  authHeader: string | null | undefined,
  team: string,
) {
  ...
  const chosen: LiveGameTeamId = team;

  await prisma.$transaction(async (transaction) => {
    await transaction.liveParticipant.update({
      where: { id: participantId },
      data: { team: chosen, lastSeenAt: new Date() },
    });
    await transaction.liveSession.update({
      where: { id: session.id },
      data: { sequence: { increment: 1 } },
  ...
  return { session: await loadSnapshot(sessionId, participantId) };
}
...
export async function leaveLiveSession(
  ...
  await prisma.$transaction(async (transaction) => {
    const left = await transaction.liveParticipant.updateMany({
      where: { id: participantId, sessionId, leftAt: null },
